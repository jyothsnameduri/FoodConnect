    ...
    <section className="py-16 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-montserrat font-bold text-2xl md:text-3xl text-[#424242] mb-4">Frequently Asked Questions</h2>
        ...
        <div className="max-w-3xl mx-auto">
          <Accordion type="single" collapsible className="w-full">
            {faqItems.map((item) => (
              <AccordionItem key={item.id} value={item.id} className="border-b border-[#E0E0E0]">
                <AccordionTrigger className="font-montserrat font-semibold text-lg py-4 px-2 text-left hover:no-underline">
                  {item.question}
                </AccordionTrigger>
                <AccordionContent className="pb-4 px-2 font-opensans text-[#424242]">
                  {item.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
